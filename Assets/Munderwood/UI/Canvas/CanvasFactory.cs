...
namespace Munderwood.UI.Canvas
{
    public class CanvasFactory
    {
        ...
        public GameObject CreateRelative(string name,int level, int x , int y)
        {
            GameObject masterCanvas;
            GameObject canvas;
            if (!this.CanvasExists("masterCanvas"))
            {
                masterCanvas = this.CreateRelativeCanvas("masterCanvas",0,0,0);
            }
            else
            {
                masterCanvas = GameObject.Find("masterCanvas");    
            ...
            if (!this.CanvasExists(name))
            {
                canvas = this.CreateRelativeCanvas(name, level,x,y,masterCanvas);
            } else
            {
                canvas = GameObject.Find(name);    
            }
        ...
        protected GameObject CreateRelativeCanvas(string name,int level , int x,int y, GameObject parentCanvas = null)
        {
